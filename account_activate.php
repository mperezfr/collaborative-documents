...
if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
    ...
    $result=$conn->prepare( "SELECT count(*) as nemail FROM users_can_participate WHERE UPPER(email)=UPPER(:email) AND hash=:hash;");
    ...
    if ($nemail==0) {
		...
		$result=$conn->prepare( "select nombre, apellidos,email, password from users_can_participate WHERE UPPER(email)=UPPER(:email);");
		...
		$conn->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
		$muser = array('email'=>$email);
		$result=$conn->prepare( "delete from users_can_participate WHERE UPPER(email)=UPPER(:email);");
		$result->execute($muser);
